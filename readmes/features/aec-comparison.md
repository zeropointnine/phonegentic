...
| L1 — Dart `feedAudio` gate | `!_speaking && !_whisper.isTtsPlaying` check before calling `feedAudio()`. Redundant on macOS (native already blocks), but provides defence-in-depth. | `agent_service.dart:903` |
| L2 — Text match | `_isEchoOfAgentResponse()`. Last resort. | `agent_service.dart` |
...
| Local STT / macOS | `isPlayingResponse` blocks event sink | `feedAudio` gate | yes |
| Local STT / Linux | **none** | `feedAudio` gate | yes |
