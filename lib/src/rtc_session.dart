...
class RTCSession extends EventManager implements Owner {
        ...
        case SipMethod.ACK:
          ...
          if (_late_sdp) {
            ...
            RTCSessionDescription answer =
                RTCSessionDescription(processedAnswer, SdpType.answer.name);
            try {
  ...
  void _setInvite2xxTimer(
      dynamic request, String? body, List<dynamic> extraHeaders) {
  ...
  void _receiveReinvite(IncomingRequest request) async {
    ...
    print(
        '[RTC] receiveReinvite() signalingState=${_connection?.signalingState}');
    ...
    void sendAnswer(String? sdp) async {
      ...
      request.reply(200, null, extraHeaders, sdp, () {
        _state = RtcSessionState.waitingForAck;
        _setInvite2xxTimer(request, sdp, extraHeaders);
        _setACKTimer();
      });
    ...
    try {
      ...
      logger.e(
          '_receiveReinvite() SDP offer failed: $error — already replied 488');
      return;
    }
  ...
  void _receiveInviteResponse(IncomingResponse? response) async {
    ...
    if (utils.test100(status_code)) {
      ...
      if (!response.body!.trimLeft().startsWith('v=')) {
        logger.e(
            '183 response body is not valid SDP (${response.body!.length} chars): '
      ...
      try {
        ...
        logger.e(
            'emit "peerconnection:setremotedescriptionfailed" [error:${error.toString()}]');
        logger.e(
            'Full processed SDP answer (${processedAnswer!.length} chars):\n$processedAnswer');
        emit(EventSetRemoteDescriptionFailed(exception: error));
      }
      ...
      if (!response.body!.trimLeft().startsWith('v=')) {
        logger.e(
            '200 response body is not valid SDP (${response.body!.length} chars): '
            '${response.body!.substring(0, response.body!.length > 200 ? 200 : response.body!.length)}');
  ...
  void _sendReinvite([Map<String, dynamic>? options]) async {
    ...
    void onSucceeded(IncomingResponse? response) async {
      print(
          '[RTC] sendReinvite onSucceeded: succeeded=$succeeded state=$_state');
      if (_state == RtcSessionState.terminated) {
        return;
      }
      ...
      if (succeeded != null) {
        print(
            '[RTC] sendReinvite onSucceeded: 2XX retransmission, skipping SDP');
        return;
      }
      ...
      try {
        print(
            '[RTC] sendReinvite: applying remote SDP answer (signalingState=${_connection!.signalingState})');
        await _connection!.setRemoteDescription(answer);
  ...
  String? _mangleOffer(String? sdpInput) {
    ...
    final RegExp directionLine = RegExp(
        r'^a=(sendrecv|sendonly|recvonly|inactive)\r?$',
        multiLine: true);
    ...
    String replaceDirection(Match m) {
      ...
      if (_localHold && !_remoteHold) {
        replacement = current == 'sendrecv'
            ? 'sendonly'
            : (current == 'recvonly' ? 'inactive' : current);
      } else if (_localHold && _remoteHold) {
        replacement = 'inactive';
      } else if (_remoteHold) {
        replacement = current == 'sendrecv'
            ? 'recvonly'
            : (current == 'sendonly' ? 'inactive' : current);
      } else {
        replacement = current;
      }
    ...
    if (_sessionTimers.refresher) {
      ...
      _sessionTimers.timer = Timer.periodic(
        ...
        (_) {
          if (_state == RtcSessionState.terminated) return;
          print(
              '[RTC] Session timer refresh firing (method=${_sessionTimers.refreshMethod} expires=$expires delayMs=$delayMs)');
          logger.d(
